...
export default async function handler(req: NextApiRequest, res: NextApiResponse<any>) {
  ...
  if (grecaptchaVerifyResult.score < 0.7) return res.status(401).json({ type: "toast-error", message: "Captcha Failed" });
  ...
  if (verifyResponse.status !== 402) {
    ...
    if (verifyResult.formatCheck === "false") return res.status(400).json({ email: "invalid email format" });
    if (verifyResult.smtpCheck === "false") return res.status(400).json({ email: "email not deliverable" });
    if (verifyResult.dnsCheck === "false") return res.status(400).json({ email: "email domain invalid" });
